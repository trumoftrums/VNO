...
class BaixeController extends Controller {
    ...
    public function get()
    {
        ...
        $arr_final = BaiXe::join('op_city', 'op_city.id', '=', 'op_bai_giu_xe.cityID')
            ->where('op_bai_giu_xe.status','<>',BaiXe::STATUS_DELETE)->select('op_bai_giu_xe.*','op_city.city_name')->orderBy('op_bai_giu_xe.updated_at', 'desc')->get()->toArray();
    ...
    public function getinfo()
    {
        ...
        if(!empty($formData['id'])){
            ...
            if(!empty($bx)){
                ...
                $result['mess'] = "Không tìm thấy bãi giữ xe";
            }
...
//            $bv->images = $image_path.substr($formData['images'],strlen($formData['images'])-32,32);
            ...
            $bv->status = BaiXe::STATUS_ACTIVE;

            $r = true;
            if(isset($formData['id']) && !empty($formData['id'])){
                $bvid = $formData['id'];
                $r = BaiXe::where('id',$bvid)->update($bv->toArray());
            }else{
