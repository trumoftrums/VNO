...
class ThuexeController extends Controller {
    ...
    public function get()
    {
        ...
        $arr_final = LendCar::join('op_city', 'op_city.id', '=', 'op_lend_car.cityID')
            ->where('op_lend_car.status','<>',LendCar::STATUS_DELETE)->select('op_lend_car.*','op_city.city_name')->orderBy('op_lend_car.updated_at', 'desc')->get()->toArray();
...
//            $content .=  '<cell><![CDATA['.$v['description'].']]></cell>';
    ...
    public function getinfo()
    {
        ...
        if(!empty($formData['id'])){
            ...
            $bvid = $formData['id'];
            $cuuho = LendCar::getLendCarbyID($bvid);
            if(!empty($cuuho)){
                ...
                $result['mess'] = "Không tìm thấy thông tin thuê xe";
            }
...
//            $bv->images = $image_path.substr($formData['images'],strlen($formData['images'])-32,32);
            ...
            $bv->status = LendCar::STATUS_ACTIVE;

            $r = true;
            if(isset($formData['id']) && !empty($formData['id'])){
                $bvid = $formData['id'];
                $r = LendCar::where('id',$bvid)->update($bv->toArray());
            }else{
    ...
    public function delete(){
        ...
        $formData = Request::all();
        $bvid = $formData['id'];
        $r = LendCar::where('id',$bvid)->update(["status"=>LendCar::STATUS_DELETE]);
        if($r){
            $result['result'] = true;
            $result['mess'] = "Xóa thông tin thuê xe thành công";
        }else{
